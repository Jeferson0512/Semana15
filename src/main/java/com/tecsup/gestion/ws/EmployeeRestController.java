...
public class EmployeeRestController {
	...
	public ResponseEntity<Void> createUser(@RequestBody Employee emp, UriComponentsBuilder ucBuilder) {
		...
		try {
			...
			employeeService.create(emp.getLogin(), emp.getPassword(), emp.getLastname(), emp.getFirstname(),
					emp.getSalary(), 12);

			emp = employeeService.findByLogin(emp.getLogin());
	...
	public ResponseEntity<Employee> updateEmp(@PathVariable("id") int id, @RequestBody Employee emp) {
		...
		try {
			...
			employeeService.update(currentEmp.getLogin(), currentEmp.getPassword(), currentEmp.getLastname(),
					currentEmp.getFirstname(), currentEmp.getSalary(), -1);
